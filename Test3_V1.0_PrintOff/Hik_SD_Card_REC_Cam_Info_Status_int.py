...
class SoftwareWatchdog:
    def __init__(self, timeout=88400):
        """
...
# Initialize software watchdog with 88400-second timeout
watchdog = SoftwareWatchdog(timeout=88400)
#--------------------------------------------------------------------------------------
...
#---------------------------- Getting Camera IP Address from Hikvision NVR------------------------------------

# Function to fetch camera IP addresses from NVR
#def get_camera_ips_from_nvr():
def hik_get_camera_ips_from_nvr():
    url = 'http://{}/ISAPI/ContentMgmt/InputProxy/channels'.format(ipaddress)
...
#            print "Parsed JSON Response:", response_json  # Debugging
            
            channels = response_json.get("InputProxyChannelList", {}).get("InputProxyChannel", [])
            #print "Extracted Channels:", channels  # Check if list exists
            
            for cam in channels:
                source_descriptor = cam.get("sourceInputPortDescriptor", {})
                ip = source_descriptor.get("ipAddress")
                if ip:
                    camera_ips.append(ip)

        else:
            # Handle XML response
           # print "Raw XML Response:", response.text  # Debugging
            
            
            try:
               root = ET.fromstring(response.content)
               ns = {'ns': root.tag.split('}')[0].strip('{')}  # Extract namespace if present
               ip_elements = root.findall(".//ns:ipAddress", ns)  # Use namespace-aware search
               #print "Found IP Elements:", ip_elements  # Debugging
    
               camera_ips = [ip.text for ip in ip_elements if ip.text]
            except ET.ParseError as xml_error:
               #print "XML Parsing Error:", xml_error
               return device_parameters_module.get_camera_ips_by_type(device_type)
        
        if camera_ips:
#            print "[INFO] Extracted Camera IPs from Hikvision NVR:", camera_ips
...
#        print "Extracted Camera IPs from Hik NVR:", camera_ips
        return camera_ips
...
def hik_main_SDcard_Record_Info():
    ...
    camera_ips = hik_get_camera_ips_from_nvr()  # Fetch camera IPs from NVR
    if camera_ips:
...
def dahua_main_SDcard_Record_Info():
    ...
    camera_ips = hik_get_camera_ips_from_nvr()
    if camera_ips:
        print("Camera IPs extracted:", camera_ips)
    else:
        print("No camera IPs found.")
