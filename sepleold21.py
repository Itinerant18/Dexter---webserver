...
password_file = "AdminPass.txt"  # Full path to password file
db_file = "sepleDB.db"
...
def home():
    if "username" in session:
        ...
        db3 = create_connection("securelink.db")
        cursor3 = db3.cursor()
...
def integration_settings():
    if "username" in session:
        ...
        try:
            ...
            # CP PLUS NVR data
            try:
                cursor.execute("SELECT ip_address, username, password, port, camera_ip FROM device_parameters WHERE device_type = 'CPPlusNVR1'")
                result= cursor.fetchone()
                if result:
