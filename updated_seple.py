...
def general():
    if 'username' in session:
        ...
        return render_template('general.html',brand_name=brand_name,branch_name=branch_name, user=user_data, logg=logss_data, user2=user_data2)
    return redirect(url_for('login'))
...
def brandname():
    if request.method == 'POST':
        ...
        # Optional hard check for max raw input length
        if len(brand_name) > 32 or len(branch_name) > 32:
            return "Each input must be 32 characters or fewer.", 400

        def format_and_center(text, width=32, pattern='+ '):
            # Replace each space with '+' and insert spaces between all characters
            formatted_chars = []
            for char in text:
                if char == ' ':
            ...
            # Add space between each character and one space before and after
            spaced_text = ' ' + ' '.join(formatted_chars) + ' '
            text_len = len(spaced_text)

            # Validate length
            if text_len > width:
                raise ValueError("Formatted text too long: '{}' ({}/{})".format(spaced_text, text_len, width))


            # Generate padding
            total_padding = width - text_len
            repeated_pattern = (pattern * ((width // len(pattern)) + 2))
            left_pad = repeated_pattern[:total_padding // 2]
            right_pad = repeated_pattern[:total_padding - len(left_pad)]

            return left_pad + spaced_text + right_pad

        try:
            # Process inputs
            name_centered = format_and_center(brand_name)
            address_centered = format_and_center(brand_name)
            branch_name_centered = format_and_center(branch_name)
            branch_address_centered = format_and_center(branch_name)
        except ValueError as e:
            return str(e), 400

        # Optional: Debug print
        print("Brand:", name_centered)
        print("Address:", address_centered)
        print("Branch:", branch_name_centered)
        print("Branch Address:", branch_address_centered)
        ...
        with open("/home/pi/TLChronosPro/branchNameAddress.txt", "w") as f:
            f.write(branch_address_centered + '\n')
...
def neteSim_data():
    if request.method == 'PUT':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE eSim SET eSim_activated=?, select_network=? WHERE ID=1", (eSim_activated, select_network))
            else:
                cursor.execute("INSERT INTO eSim (eSim_activated, select_network, ID) VALUES (?, ?, ?)", (
...
def data2_lan():
    if request.method == 'PUT':
        ...
        if db:
            ...
            existing = cursor.fetchone()
            if existing:
                cursor.execute("UPDATE lan SET network_led_sts=?, wireless_lan=?,ip_module=?, static_or_dynamic=? WHERE ID=1", (
                    network_led_sts, wireless_lan, ip_module, static_or_dynamic))
            else:
                cursor.execute("INSERT INTO lan (network_led_sts,wireless_lan,ip_module,static_or_dynamic) VALUES (?,?,?,?)", (
                    network_led_sts, wireless_lan, ip_module, static_or_dynamic))
        db.commit()
        cursor.close()
        db.close()
...
def acces_token():
    if request.method == 'POST':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE modem_parameters SET access_token=? WHERE id=1", (access_token,))
            else:

                cursor.execute(
                    "INSERT INTO modem_parameters (access_token) VALUES (?)", (access_token,))
    db.commit()
    cursor.close()
    db.close()
...
def x509():
    if request.method == 'POST':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE modem_parameters SET x509=? WHERE id=1", (x509,))
            else:
...
def mqtt_basic():
    ...
    if request.method == 'POST':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE modem_parameters SET client_id=?,user_name=?,password=?  WHERE id=1", (clintId, username, password))
            else:
                cursor.execute(
                    "INSERT INTO modem_parameters (client_id, user_name, password) VALUES (?,?,?)", (clintId, username, password))
    db.commit()
    cursor.close()
    db.close()
    return redirect(url_for('connection'))
...
def sendData():
    if request.method == 'POST':
        ...
        if db:
            ...
            existing = cursor.fetchall()
            if existing:
                cursor.execute("UPDATE provision SET E_deviceName=?, E_id=?,E_userName=?,E_passWord=?", (
                    deviceName, id, userName, password))
            else:
                cursor.execute("INSERT INTO provision (E_deviceName, E_id,E_userName,E_passWord) VALUES(?,?,?,?)", (
                    deviceName, id, userName, password))
    db.commit()
    cursor.close()
    db.close()
    ...
    if request.method == 'POST':
        ...
        if db:
            ...
            if existing:
                ...
                cursor.execute(
                    "INSERT INTO provision (etn_deviceName) VALUES(?)", (ethernetDeviceName,))
    db.commit()
    cursor.close()
    db.close()
    return redirect(url_for('provision'))
...
def data_active_integration():
    if request.method =='PUT':
        ...
        if db:
            cursor = db.cursor()
            cursor.execute("SELECT * FROM active_integration_external_device WHERE ID=1")
            existing = cursor.fetchall()
            if existing:
                cursor.execute(
                    "UPDATE active_integration_external_device SET active_integration_on_off_bit=? WHERE ID=1", (active_integration_external,))
            else:
                cursor.execute(
                    "INSERT INTO active_integration_external_device (active_integration_on_off_bit) VALUES (?)", (active_integration_external,))
        db.commit()
        cursor.close()
        db.close()
        return ({"data": "send"})
...
def get_active_integration_hikvidion():
    if 'username' in session:
        db = create_connection("logical_params_active_integration.db")
        if db:
            cursor = db.cursor()
...
def data_active_integration_hikvidion():
    if request.method =='PUT':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE parameters SET value=? WHERE id=1", (active_integration_external_hk,))
            else:
                cursor.execute(
                    "INSERT INTO parameters (value) VALUES (?) where id=1", (active_integration_external_hk,))
        db.commit()
        cursor.close()
        db.close()
        return ({"data": "send"})
...
def data_active_integration_ac():
    if request.method =='PUT':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE parameters SET value=? WHERE id=2", (active_integration_external_ac,))
            else:
                cursor.execute(
                    "INSERT INTO parameters (value) VALUES (?) where id=2", (active_integration_external_ac,))
        db.commit()
        cursor.close()
        db.close()
        return ({"data": "send"})
...
def data_active_integration_dh():
    if request.method =='PUT':
        ...
        if db:
            ...
            if existing:
                cursor.execute(
                    "UPDATE parameters SET value=? WHERE id=3", (active_integration_external_dh,))
            else:
                cursor.execute(
                    "INSERT INTO parameters (value) VALUES (?) where id=3", (active_integration_external_dh,))
        db.commit()
        cursor.close()
        db.close()
        return ({"data": "send"})
...
def device_credential():
    if 'username' in session:
        ...
        try:
            cursor.execute(
                "SELECT * FROM device_credentials WHERE type='esim' AND id=1")
        ...
        try:
            cursor.execute(
                "SELECT * FROM device_credentials WHERE type='ethernet' AND id=1")
            result = cursor.fetchone()
            if result:
                # Convert tuple to dictionary
...
def save_device_credentials():
    ...
    try:
        ...
        ac= request.form.get("access_token")
        if ac :
            # device_type = 'esim'
            access_token = request.form.get('access_token')
            clint_id = request.form.get('clientId')
            username = request.form.get('user_name')
            password = request.form.get('password')
            devicename = request.form.get('solId')
            # device_secret = request.form.get('deviceSecret')
            ...
            try:
                ...
                cursor.execute(
                    "SELECT * FROM modem_parameters WHERE id=1")
                existing = cursor.fetchone()
                ...
                if existing:
                    cursor.execute('''
                        ...
                        SET access_token=?, client_id=?, user_name=?, password=? ,gsm_modem_mode=?,network_type=?,device_name=?
                        WHERE id=1
                    ''', (access_token, clint_id, username, password,"esim","gsm",devicename))
                    
                else:
                    cursor.execute('''
                        INSERT INTO modem_parameters 
                        (id, access_token, client_id, user_name, password ,gsm_modem_mode,network_type,device_name) 
                        VALUES (1, ?, ?, ?, ?, ?, ?, ?)
                    ''', ("1",access_token, clint_id, username, password,"esim","gsm",devicename))
                ...
                if db2:
                    ...
                    cursor2.execute(
                        "INSERT INTO systemLogs (date, message) VALUES (datetime('now', 'localtime'), ?)", (log_message,))
                    db2.commit()
                    cursor2.close()
                    db2.close()
...
def integration_settings():
    if 'username' in session:
        ...
        return render_template('integrationSettings.html',
                               ...
                               c1=hikvision_nvr_data['camera_ip'], c2=hikvision_bacs_data['camera_ip'], c3=dahua_nvr_data['camera_ip'])  # <--- this will be list now
    print("{hikvision_nvr_data['camera_ip']}//{hikvision_bacs_data['camera_ip']}//{dahua_nvr_data['camera_ip']}")
    return redirect(url_for('login'))
    ...
    try:
        ...
        try:
            ...
            if existing:
                ...
                cursor.execute(
                    update_sql, ( ip_address, username, password, port_number,camera_ip,device_type))
                print("kk")
            else:
                insert_sql = "INSERT INTO device_parameters (device_type, ip_address, username, password, port,camera_ip) VALUES (?,?,?,?,?,?)"
                cursor.execute(
                    insert_sql, (device_type, ip_address, username, password, port_number,camera_ip))
