...
provision_status = {"result": None}  # global dictionary for tracking
...
def lan():
    if "username" in session:
        # Check if user has access to LAN setup
        ...
        cursor2 = db2.cursor()
        cursor.execute("SELECT * FROM users WHERE username=?", (username,))
        cursor2.execute("SELECT * FROM Settings ORDER BY id ASC")
        user2_data = cursor2.fetchall()
        # print(json.dumps(user2_data))
        user_data = cursor.fetchone()
        cursor.close()
        db.cursor()
        ...
        cursor3.close()
        db3.close()
        if session.get('run_function'):
            configureStaticNetwork()
            session['run_function'] = False
        return render_template(
            "Lan.html", user=user_data, logg=logss_data, user2=user2_data
        )
    return redirect(url_for("login"))
    ...
    if request.method == 'POST':
        ...
        if db:
            ...
            for setting_name, setting_value in data.items():
                cursor.execute("SELECT setting_value FROM Settings WHERE setting_name IN (?, ?, ?, ?, ?, ?, ?)",('Set IP Address','Set Port Number','Subnet mask','Gateway','preferred_dns_server','alternate_dns_server','APN Settings',))
...
def provision():
    ...
    # ✅ Show provisioning result if available
    if provision_status["result"] == "success":
        flash("✅ Device provisioned successfully!")
        provision_status["result"] = None  # reset after showing
    elif provision_status["result"] == "failed":
        flash("❌ Device provisioning failed.")
        provision_status["result"] = None
    elif provision_status["result"] and provision_status["result"].startswith("error"):
        flash("❌ Provisioning failed due to error: " + provision_status["result"])
        provision_status["result"] = None
...
def setprovisiondevice():
    def run_provision():
        try:
            ...
            if data == "success":
                provision_status["result"] = "success"
            else:
                provision_status["result"] = "failed"
        except Exception as e:
            provision_status["result"] = "error: " + str(e)

    threading.Thread(target=run_provision, daemon=True).start()

    flash("Device provisioning started. Please wait...")
